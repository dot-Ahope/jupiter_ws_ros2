...
def generate_launch_description():
    ...
    # 라이다 시리얼 포트 파라미터 추가
    lidar_port = LaunchConfiguration('lidar_port', default='/dev/lidar_port')
    ...
    # RPLidar S2L 설정 
    # S2L: 1000000 baudrate, DenseBoost scan mode
    sllidar_node = Node(
        ...
        parameters=[{
            ...
            'serial_baudrate': 1000000,     # S2L: 1000000 (A1은 115200)
            'frame_id': 'laser',
            'inverted': False,
            'angle_compensate': True,
            'scan_mode': 'Standard',        # S2L 스캔 모드: DenseBoost->Standard (부하 감소)
        }],
        output='screen'
    )
    ...
    # Foxglove Bridge 실행 - launch argument로 제어
    use_foxglove_arg = DeclareLaunchArgument(
        'use_foxglove',
        default_value='true',
        description='Use Foxglove Bridge'
    )
    
    # ⭐ jupiter_nav 패키지의 RViz 설정 사용 (sllidar_ros2가 아닌)
    jupiter_nav_dir = get_package_share_directory('jupiter_nav')
    rviz_config_file = os.path.join(jupiter_nav_dir, 'rviz', 'sllidar.rviz')
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
    ...
    jupiter_driver_node = Node(
        ...
        executable='jupiter_driver_compensated', #jupiter_driver_compensated
        name='jupiter_driver',
        output='screen',
    ...
    # RF2O Laser Odometry Node (Lidar Odometry)
    rf2o_node = Node(
