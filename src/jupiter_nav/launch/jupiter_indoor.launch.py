...
def generate_launch_description():
    ...
    # KISS-ICP Config
    kiss_config_file = os.path.join(jupiter_nav_dir, 'config', 'kiss_icp_light.yaml')
    ...
    # =========================================================
    # RealSense D455f Configuration (Sensor Fusion)
    # =========================================================
    
    # RealSense Launch including filters for CPU optimization
    realsense_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('realsense2_camera'), 'launch', 'rs_launch.py')
        ),
        launch_arguments={
            'pointcloud.enable': 'true',           # Enable PointCloud2 for Nav2
            'decimation_filter.enable': 'true',    # Reduce resolution (Save CPU)
            'spatial_filter.enable': 'true',       # Fill holes
            'temporal_filter.enable': 'true',      # Reduce noise over time
            'align_depth.enable': 'true',          # Align depth to RGB
            'depth_module.profile': '640x480x30',  # Moderate resolution
            'rgb_camera.profile': '640x480x30'
        }.items()
    )

    # Static TF: Robot Base -> Camera Link
    # TODO: Adjust x/y/z/r/p/y to match your actual mounting position!
    # Current Assumption: Mounted 20cm forward (x=0.2), 20cm up (z=0.2)
    tf_base_camera = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='tf_base_camera',
    ...
    # 6. Static TF for GPS (Optional for backup, but kept for consistency)
    # Transform from base_link to gps_link
    # tf_base_gps = Node(...) # Removed in backup
    ...
    return LaunchDescription([
        ...
        tf_laser_velodyne,
        realsense_launch,        # RealSense Camera
        tf_base_camera,          # RealSense TF
    ])
