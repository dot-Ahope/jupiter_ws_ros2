...
class Jupiter_ARM:
    ...
    def get_current_angles(self):
        # 基本角度值 Basic angle values
        base_angles = [225.0, 45.0, 90.0]  # 默认角度值 Default angles
        
        # 应用偏移量 Apply offsets
        current_angles = [
            base_angles[0] + self.__offset_7,
            base_angles[1] + self.__offset_8,
            base_angles[2] + self.__offset_9
        ]
        
        return current_angles

if __name__ == '__main__':
    bot_arm = Jupiter_ARM(debug=False)
    offset = bot_arm.get_arm_offset()
    print("offset:", offset)
    angles = bot_arm.get_current_angles()
    print("current angles:", angles)
    del bot_arm
    print('arm_jupiter.py killed')
